...
- [ ] Extend `parse_context_config` method or create `parse_vm_context` method in `bot_manager.rb` (AC: 1, 5)
  - [ ] Locate attack parsing code (around line 1255-1279 in bot_manager.rb)
  - [ ] Add parsing for optional `<vm_context>` element within `<attack>`
  - [ ] Follow existing `parse_context_config` pattern (lines 1627-1750)
  - [ ] Store VM context config in attack hash: `attack_data['vm_context'] = {...}`
- [ ] Implement parsing for `<bash_history>` element with attributes (AC: 2)
  - [ ] Parse `path` attribute (default: `~/.bash_history`)
  - [ ] Parse `limit` attribute (number, optional)
  - [ ] Parse `user` attribute (string, optional)
  - [ ] Store as hash: `{ path: '~/.bash_history', limit: 50, user: 'student' }`
  - [ ] Handle missing element (default to nil/disabled)
- [ ] Implement parsing for `<commands>` element (AC: 3)
  - [ ] Parse multiple `<command>` child elements
  - [ ] Extract command text from each `<command>` element
  - [ ] Store as array: `['ps aux', 'netstat -tuln', 'whoami']`
  - [ ] Handle empty `<commands>` element (empty array)
  - [ ] Handle missing element (nil or empty array)
- [ ] Implement parsing for `<files>` element (AC: 4)
  - [ ] Parse multiple `<file>` child elements
  - [ ] Extract `path` attribute from each `<file>` element
  - [ ] Store as array: `['/etc/passwd', './config.txt']`
  - [ ] Handle empty `<files>` element (empty array)
  - [ ] Handle missing element (nil or empty array)
- [ ] Ensure default behavior (no VM context when config missing) (AC: 6)
  - [ ] Attacks without `<vm_context>` element continue to work unchanged
  - [ ] Bot state doesn't include `vm_context` key if not configured
  - [ ] No errors logged when VM context config is missing
- [ ] Add configuration examples to XML config files (AC: 7)
  - [ ] Update `config/example_stage_aware_context.xml` with VM context example
  - [ ] Add example showing bash_history with attributes
  - [ ] Add example showing commands configuration
  - [ ] Add example showing files configuration
  - [ ] Add example showing all VM context elements combined
- [ ] Create or update XML schema validation (AC: 8)
  - [ ] Check if `hackerbot_schema.xsd` exists and update it
  - [ ] Add `<vm_context>` element definition with optional sub-elements
  - [ ] Define `<bash_history>` element with attributes
  - [ ] Define `<commands>` and `<command>` elements
  - [ ] Define `<files>` and `<file>` elements
  - [ ] Ensure schema validates existing configs still work
